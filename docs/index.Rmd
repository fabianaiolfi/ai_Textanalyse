...
embeddings_clean <- ai_clean %>% 
  select(id, tx_clean, rubrik_graph, year, ht)
...
rubrik_id <- ai_clean %>% select(id, rubrik_graph, ht)
...
ggplot(pca_embedding_graph, aes(x = PC2, y = PC1, color = rubrik_graph)) +
  ...
  theme(panel.grid.major.x = element_line(size=.1, color="black"),
        ...
        legend.position="bottom",
        legend.title=element_blank()) +
  geom_label_repel(data = pca_embedding_graph, aes(x = PC2, y = PC1, label = ht))
```
...
ggplot(pca_embedding_graph, aes(x = PC2, y = PC1, color = rubrik_graph)) +
  ...
  theme(panel.grid.major.x = element_line(size=.1, color="black"),
        ...
        legend.position="bottom",
        legend.title=element_blank()) +
  geom_label_repel(data = subset(pca_embedding_graph, PC2 > 0.16 | PC1 < -0.25), aes(x = PC2, y = PC1, label = ht))
```
...
### Titel im Vector Space

```{r, fig.height=10, cache=T}
# Document Embeddings
title_embeddings_clean <- ai_clean %>% 
  select(id, ht, rubrik_graph, year)

title_embeddings_clean$ht <- tolower(title_embeddings_clean$ht)
title_embeddings_clean$ht <- gsub("[^[:alpha:]]", " ", title_embeddings_clean$ht)
title_embeddings_clean$ht <- gsub("[[:space:]]+", " ", title_embeddings_clean$ht)
title_embeddings_clean$ht <- trimws(title_embeddings_clean$ht)

title_embeddings_clean <- title_embeddings_clean %>% 
  rename("doc_id" = id) %>% 
  rename("text" = ht)

model_title_embedding <- paragraph2vec(x = title_embeddings_clean,
                                 type = "PV-DM",
                                 dim = 200, # 40
                                 iter = 800,  # 400
                                 min_count = 50,
...
title_embedding <- as.matrix(model_title_embedding, which = "docs")
title_embedding <- as.data.frame(title_embedding)

pca_title_embedding <- prcomp(title_embedding)

pca_title_embedding_graph <- pca_title_embedding$x %>% as.data.frame()
pca_title_embedding_graph <- tibble::rownames_to_column(pca_title_embedding_graph, "id")
pca_title_embedding_graph$id <- as.numeric(pca_title_embedding_graph$id)
rubrik_id <- ai_clean %>% select(id, rubrik_graph, ht)
pca_title_embedding_graph <- pca_title_embedding_graph %>% left_join(rubrik_id, by = "id")

ggplot(pca_title_embedding_graph, aes(x = PC2, y = PC1, color = rubrik_graph)) +
  geom_point(size = 2, alpha = 0.5) +
  ylab(" ") +
  xlab(" ") +
  scale_color_manual(values = c("#8aabfd", "#4fbbae", "#374e8e", "#ac004f", "#df7c18", "#e3b13e")) +
...
ggplot(pca_title_embedding_graph, aes(x = PC2, y = PC1, color = rubrik_graph)) +
  ...
  theme(panel.grid.major.x = element_line(size=.1, color="black"),
        ...
        legend.position="bottom",
        legend.title=element_blank()) +
  geom_label_repel(data = subset(pca_title_embedding_graph, PC2 > -0.5 | PC1 > 0.25), aes(x = PC2, y = PC1, label = ht))
```
...
ggplot(pca_title_embedding_graph, aes(x = PC2, y = PC1, color = rubrik_graph)) +
  ...
  theme(panel.grid.major.x = element_line(size=.1, color="black"),
        ...
        legend.position="bottom",
        legend.title=element_blank()) +
  geom_label_repel(data = subset(pca_title_embedding_graph, PC2 > 0.5 | PC1 > -0.325), aes(label = ht))
```
...
ggplot(pca_title_embedding_graph, aes(x = PC2, y = PC1, color = rubrik_graph)) +
  ...
  theme(panel.grid.major.x = element_line(size=.1, color="black"),
        ...
        legend.position="bottom",
        legend.title=element_blank()) +
  geom_label_repel(data = subset(pca_title_embedding_graph, PC2 > -0.2 | PC1 > 0.45), aes(label = ht))
```
...
## Zeit-Analyse
```{r eval=FALSE, fig.height=10, cache=TRUE, include=FALSE}
...
## Textanalyse: Wort- und Satzlänge
